...
if curl -s --head curl https://snapshots.polkachu.com/testnet-snapshots/nillion/nillion_3264877.tar.lz4 | head -n 1 | grep "200" > /dev/null; then
  curl https://snapshots.polkachu.com/testnet-snapshots/nillion/nillion_3264877.tar.lz4 | lz4 -dc - | tar -xf - -C $HOME/.nillionapp
    else
  echo no have snap
fi
