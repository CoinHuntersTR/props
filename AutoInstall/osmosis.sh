...
ExecStart=$(which osmosis) start --home $HOME/.osmosisd
Restart=on-failure
RestartSec=5
LimitNOFILE=65535
[Install]
...
junctiond tendermint unsafe-reset-all --home $HOME/.osmosisd
...
sudo systemctl restart osmosisd && sudo journalctl -u osmosisd -f
